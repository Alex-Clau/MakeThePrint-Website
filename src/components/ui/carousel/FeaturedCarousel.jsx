...
function FeaturedCarousel() {
        ...
        if (view === 'mobile') {
            return (
                <div className="flex flex-col gap-2">
                    ...
                    <button onClick={() => setSelectedItemId(item.id)} className="px-2 py-1 bg-blue-800 hover:bg-blue-700 text-white font-semibold rounded-md text-xs transition-all">
                        Details
                    </button>
                </div>
            );
        ...
        return (
            <div className="flex flex-col md:flex-row gap-3 md:gap-6 items-start md:items-center rounded-lg">
                ...
                <div className="flex-1 flex flex-col justify-center">
                    ...
                    <p className="text-white/80 text-sm md:text-lg mb-0.5 md:mb-1">${item.price}</p>
                    <p className="text-white/60 capitalize text-xs md:text-sm mb-2 md:mb-4">{item.category}</p>
    ...
    return (
        <>
            <Carousel items={featuredItems} itemsPerSlide={3} renderItem={renderItem} />
            {selectedItemId && (
                <ModalItemDetails
                    showModal={true}
